...
// BODY
  ...
  cpBodyArbiterIteratorFunc = procedure( body : PcpBody; arbiter : PcpArbiter; data: Pointer); cdecl;
...
// ARBITER
  ...
  cpBodyEachArbiter : procedure( body : PcpBody; func: cpBodyArbiterIteratorFunc; data: Pointer);
...
{$ELSE}
  ...
  procedure cpBodyEachArbiter( body : PcpBody; func: cpBodyArbiterIteratorFunc; data: Pointer); cdecl;{$IFDEF CHIPMUNK_LINK} external libChipmunk; {$ELSE} external; {$ENDIF}
...
begin
  ...
  if cpLib <> {$IFDEF UNIX} nil {$ENDIF} {$IFDEF WINDOWS} 0 {$ENDIF} Then
    begin
      ...
      cpBodyEachArbiter := dlsym( cpLib, 'cpBodyEachArbiter' );
